...
def test_builds_architecture_identifiers_and_object_key() -> None:
    ...
    stream = build_derived_stream_id("win123456", "T0")
    image = build_image_id(stream, timestamp)

    assert stream == "win123456T0"
    assert image == "20260612T143512Z_win123456T0.jpg"
    assert build_object_key("T0", "win", image, timestamp) == (
        "T0/win/2026/06/12/14/20260612T143512Z_win123456T0.jpg"
    )
